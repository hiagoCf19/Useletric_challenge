...
const DeviceItem = ({ device }) => {
  ...
  useEffect(() => {
    ...
    // alterei para que o effect fosse chamado quando os dados mudassem
  ...
  const generateCSV = () => {
    ...
    const nonTemperatureSensors = Object.keys(transformedData).filter(
      (key) => key !== 'temperature',
    )
    ...
    const headers = [
      'Created_at',
      ...nonTemperatureSensors,
      ...Object.keys(temperatureSensors).map((sensorId) => `temperature_${sensorId}`),
    ]
    const csvData = []
  ...
  const filteredData = deviceData
    ? filterDataByRange(deviceData, timeRange)
    : { timeRange: null, deviceData: [] }
      ...
      <CRow>
        {Object.keys(transformedData)
          ...
          .map((key) => {
            ...
            const lastData = deviceData && deviceData.length > 0 ? deviceData[0].data[key] : null
            const formattedLastData = lastData !== null ? parseFloat(lastData).toFixed(2) : null
            // Encontrando a última transmissão
            const lastTransmission =
              deviceData && deviceData.length > 0 ? deviceData[0].created_at : null
      ...
      <CRow className="mt-4">
        {transformedData.temperature &&
          Object.keys(transformedData.temperature).map((sensorId) => {
            ...
            const sensorInfo = deviceSensor.find(
              (sensor) => sensor.id_sensor_type.toString() === sensorId,
            )
            ...
            return (
              <CCol xs={6} key={sensorId}>
                <CCard className="mb-4 ">
                  ...
                  <CCardBody>
                    <VariableChart
                      deviceData={transformedData.temperature[sensorId]
                        .slice()
                        .reverse()
                        .map((entry) => ({
                          created_at: formatDate(parseDate(entry.created_at)),
